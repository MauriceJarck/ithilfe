...
def test_p_register_validate_open(main_window, qtbot, create_csv):
    ...
    assert main_window.stackedWidget.currentIndex() == 0
    main_window.open(True)
    count = main_window.pViewTable.rowCount()
    # register new
    qtbot.keyClick(main_window, "n", modifier=QtCore.Qt.ControlModifier)
    assert main_window.stackedWidget.currentIndex() == 1
    main_window.inUsername.setText("maurice")
    ...
    main_window.inComboboxOs.setCurrentIndex(1)
    qtbot.mouseClick(main_window.btRegister, QtCore.Qt.LeftButton)
    assert main_window.stackedWidget.currentIndex() == 0
    # get len(already displayed registrations) after new reg
    assert main_window.pViewTable.rowCount() == count + 1
    count += 1
    qtbot.keyClick(main_window, "n", modifier=QtCore.Qt.ControlModifier)
    assert main_window.stackedWidget.currentIndex() == 1
    main_window.inUsername.setText("maurice")
    main_window.inDevicename.setText("3")
    main_window.inComboboxDevicetype.setCurrentIndex(1)
    main_window.inComboboxOs.setCurrentIndex(2)
    qtbot.mouseClick(main_window.btRegister, QtCore.Qt.LeftButton)
    assert main_window.stackedWidget.currentIndex() == 0
    # get len(already displayed registrations) after new reg
    assert main_window.pViewTable.rowCount() == count + 1
...
def test_search(main_window, qtbot, create_csv):
    ...
    qtbot.mouseClick(main_window.btCancelSearch, QtCore.Qt.LeftButton)
    assert main_window.stackedWidget.currentIndex() == 0
    ...
    assert main_window.pSearchTable.rowCount() == 1
...
def test_new(main_window, qtbot):
    ...
    assert main_window.stackedWidget.currentIndex() == 4
    main_window.inNewFilename.setText("newCSV")

    qtbot.mouseClick(main_window.btCreate, QtCore.Qt.LeftButton)
    assert main_window.stackedWidget.currentWidget() == main_window.pView
